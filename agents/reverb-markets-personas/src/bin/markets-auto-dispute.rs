//! `markets-auto-dispute` persona binary. Tool namespace: `mkad_*`. Scoped to
//! file_dispute + reads. Allowed contracts: Reverb Markets Operator + RefundProtocolFixed
//! (the dispute primitive on Arc testnet).
...
use reverb_markets_arc_fs::tools::{
    file_dispute, read_market_state, read_settlement_history,
};
use reverb_markets_personas::AutoDisputePersona;

const NAMESPACE: &str = "mkad";
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("markets-auto-dispute: fatal: {e}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let persona = AutoDisputePersona::new(&variant);
